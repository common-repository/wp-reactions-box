...
function hkp_feelbox_save_results( $vote, $postid ) {
	...
	if ( $vote && in_array( $vote, $moods ) ) {
		...
		$voteFields = $wpdb->get_row(
				$wpdb->prepare(
						"SELECT * FROM " . $table_name . " WHERE ID=%s", $postid
				)
		);
		...
		if ( $voteFields ) {
			...
			$row = $wpdb->update(
					$table_name, array( 'ID' => $postid, $vote => $votecount + 1, 'time' => current_time( 'mysql' ) ), array( 'ID' => $postid )
			);
		} else {
			$row = $wpdb->insert(
					$table_name, array( 'ID' => $postid, $vote => 1, 'time' => current_time( 'mysql' ) )
			);
		}
